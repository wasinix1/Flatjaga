...
class IdMaintainer:
    ...
    def update_expose_description(self, listing_id, crawler, description):
        """
        Update an existing expose with the full description text.
        Merges the description into the existing expose JSON.

        Args:
            listing_id: The listing ID
            crawler: The crawler name (e.g., 'Willhaben')
            description: The description text to add
        ...
        try:
            ...
            # First, get the existing expose
            cur.execute('SELECT details FROM exposes WHERE id = ? AND crawler = ?',
                       (int(listing_id), crawler))
            row = cur.fetchone()

            if row is None:
                logger.warning(f"Cannot update description: expose {listing_id} ({crawler}) not found in database")
                return False

            # Parse existing details
            expose_data = json.loads(row[0])

            # Add/update description field
            expose_data['description'] = description

            # Update the database record
            cur.execute('UPDATE exposes SET details = ? WHERE id = ? AND crawler = ?',
                       (json.dumps(expose_data), int(listing_id), crawler))
            self.get_connection().commit()

            logger.debug(f"Updated description for expose {listing_id} ({crawler})")
            return True

        except (lite.Error, KeyError, ValueError) as e:
            logger.error(f"Database error updating description for expose {listing_id}: {e}")
            return False

    def get_expose_by_id(self, listing_id, crawler):
        """
        Get an expose by ID and crawler name.

        Args:
            listing_id: The listing ID
            crawler: The crawler name

        Returns:
            dict: The expose data, or None if not found
        """
        try:
            cur = self.get_connection().cursor()
            cur.execute('SELECT details FROM exposes WHERE id = ? AND crawler = ?',
            ...
            logger.error(f"Database error getting expose {listing_id}: {e}")
            return None

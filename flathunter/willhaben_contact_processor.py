...
class WillhabenContactProcessor:
    ...
    def __init__(self, config, telegram_notifier=None, id_watch=None, session_manager=None):
        ...
        self.mietprofil_mode = config.get('willhaben_mietprofil_mode', 'disabled')
    ...
    def _init_bot(self, use_headless=None, increase_delays=False, _retry_with_visible=True):
        ...
        try:
            logger.info(f"Starting willhaben contact bot (headless={headless_mode}, delays={delay_min}-{delay_max}s, mietprofil={self.mietprofil_mode})...")
            self.bot = WillhabenContactBot(
                headless=headless_mode,
                delay_min=delay_min,
                delay_max=delay_max,
                mietprofil_mode=self.mietprofil_mode
            )
            self.bot.start()
